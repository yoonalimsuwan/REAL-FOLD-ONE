...
#   - Positional restraints for partial refinement
# =============================================================================
...
# =============================================================================
# 1.1 Ramachandran sampler for CDR loop modelling
# =============================================================================
# Simplified Ramachandran probability map (10° bins) for general amino acids
# Weighted towards α‑helix and β‑sheet regions.
...
class OpenMMEnergyCalculator:
    ...
    def __init__(self, system: mm.System, topology: app.Topology,
        ...
        # Cache the full coordinate array (numpy, nm) to avoid Vec3 creation
    ...
    def compute(self, solute_coords: torch.Tensor) -> Tuple[float, torch.Tensor]:
        # Update only solute part of the cached numpy array
...
# =============================================================================
# 4. Fast Neighbour List Manager (O(N) grid with torch operations)
# =============================================================================
class FastNeighborList:
    """Vectorized grid neighbour list using PyTorch scatter operations."""
    def __init__(self, coords: torch.Tensor, cutoff: float):
        ...
        # Precompute 3D offsets for 27 neighboring cells
        offsets = torch.stack(torch.meshgrid(
            torch.tensor([-1,0,1], device=self.device),
            torch.tensor([-1,0,1], device=self.device),
            torch.tensor([-1,0,1], device=self.device),
            indexing='ij'), dim=-1).reshape(-1, 3)
        self.offset_linear = (offsets * self.stride).sum(dim=-1)  # 27
        # Create inverse mapping for fast lookup of cell linear index
        self.lin_to_pos = {l.item(): i for i, l in enumerate(unique_lin)}
    ...
    def query(self) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        edges = []
        dists = []
        for i, lin in enumerate(self.unique_lin):
            cell_atoms = torch.arange(self.cell_start_idx[i], self.cell_end_idx[i], device=self.device)
            for off_lin in self.offset_linear:
                neigh_lin = lin + off_lin
                # find neighbor cell
                if neigh_lin.item() not in self.lin_to_pos:
                    continue
                j = self.lin_to_pos[neigh_lin.item()]
                if lin > neigh_lin:  # avoid duplicates
                    continue
                neigh_atoms = torch.arange(self.cell_start_idx[j], self.cell_end_idx[j], device=self.device)
                if lin == neigh_lin:
                    # same cell: only upper triangle
                ...
                mask = d < self.cutoff
                if mask.any():
                    edges.append(torch.stack([self.sorted_idx[I[mask]], self.sorted_idx[J[mask]]], dim=0))
                    dists.append(d[mask])
        if edges:
...
class SOCController(nn.Module):
    ...
    def compute_soc_energy(self, ca: torch.Tensor, alpha: torch.Tensor,
        ...
        K = self.kernel(d)  # already includes exponential decay with learnable scale
        E = -a * K
        return w_soc * E.sum()
...
# =============================================================================
# 7. DNA Origami – realistic double‑helix model
# =============================================================================
def build_double_helix(seq: str,
    ...
    for i in range(L):
        ...
        # strand 1 at angle ang, strand 2 at ang + pi (180°) opposite side
...
class WireframeOrigami:
    ...
    def build_3d_model(self):
        ...
        for idx in range(len(path) - 1):
            ...
            # generate double helix for this segment
            seg_seq = "A" * n_bp  # placeholder, actual sequence could be from scaffold
            strand1, strand2 = build_double_helix(seg_seq, rise=3.38, twist=36.0, radius=10.0)
            # concatenate both strands
            helix_local = torch.cat([strand1, strand2], dim=0)  # (2*n_bp, 3)
            ...
            seq_all += seg_seq * 2  # both strands
    ...
    def export_full_atom_pdb(self, filename):
        ...
        if c4_coords.shape[0] == 0:
            return
        # Write a simple PDB with C4' atoms
...
class CDRLoopModeler:
    ...
    def _rebuild_ca_trace(self, coords_ab: torch.Tensor, loop_start: int, loop_end: int) -> torch.Tensor:
        """
        Rebuild CA trace using backbone dihedral angles sampled from
        Ramachandran distribution. Uses ideal CA‑CA distance (3.8 Å)
        and CA‑CA‑CA angle (130°). The loop is built sequentially.
        """
        ...
        p_prev = ca_trace[loop_start - 1]
        p_cur = ca_trace[loop_start]
        v_old = p_cur - p_prev
        v_old = v_old / (v_old.norm() + 1e-8)
        # For each new point we need phi/psi. We'll use phi to rotate the new bond.
        for i in range(loop_start + 1, loop_end):
            # sample (φ, ψ) pair; φ rotates the (i-1,i) bond relative to previous.
            phi_psi = sample_rama_phi_psi(1)[0]  # [phi, psi] in degrees
            phi = math.radians(phi_psi[0])
            # axis is the direction from previous CA to current CA
            axis = (ca_trace[i-1] - ca_trace[i-2])
            axis = axis / (axis.norm() + 1e-8)
            # ideal direction without dihedral: rotate axis by pi - angle_ca
            # first, compute normal vector: cross(axis, (p_cur - p_prev)) ?
            # Better: use Rodrigues to rotate axis by (pi - angle_ca) around an arbitrary perpendicular
            ...
            # apply phi rotation around the axis
...
class MDEngine:
    """
    Production‑grade molecular dynamics engine powered by OpenMM.
    Supports NVT, NPT, and Langevin integrators with checkpointing.
    """
    def __init__(self,
                 system: mm.System,
...
class RefinementConfig:
    ...
    restraint_atoms: Optional[List[int]] = None
    restraint_k: float = 10.0  # kcal/mol/Å²
    restraint_target: Optional[torch.Tensor] = None
    adaptive_neighbor_rebuild: bool = True
    rebuild_displacement_thresh: float = 1.0  # Å
...
class RefinementEngine(nn.Module):
    ...
    def _auto_chain_boundaries(self, ca_coords: torch.Tensor) -> List[int]:
        # Use CA distance > 5 Å or chain ID changes to detect boundaries
    ...
    def refine(self, pdb_file: str, steps: Optional[int] = None,
        ...
        # Auto‑detect chain boundaries if not provided
        ...
        # Prepare restraint if any
        restraint_k = self.cfg.restraint_k
        has_restraint = self.cfg.restraint_atoms is not None and self.cfg.restraint_target is not None
        if has_restraint:
            restraint_target = self.cfg.restraint_target.to(self.device)
            restraint_mask = torch.tensor(self.cfg.restraint_atoms, device=self.device, dtype=torch.long)
        ...
        for step in range(steps):
            ...
            # Adaptive neighbor list rebuild
    ...
    def export_pdb(self, solute_coords_np: np.ndarray, output_file: str):
        ...
        if HAS_BIOTITE:
            # Use biotite to write with proper connectivity
            # Reconstruct a biotite AtomArray from topology and coordinates
            try:
                atoms = []
        ...
        # Fallback: use OpenMM's PDBFile writer
        pos_nm = full * 0.1
...
def main():
    ...
    refine_parser.add_argument('--restraint-json', type=str, help='JSON with "atoms" and "target" for harmonic restraints')
    ...
    validate_parser = sub.add_parser('validate', help='Run a quick refinement and check energy improvement')
    ...
    if args.command == 'refine':
        ...
        print(f"α={alpha:.4f}, λ={lambd:.4f}, scale={scale:.4f}")
        ...
        if not HAS_OPENMM:
            print("OpenMM is required for MD simulation.")
            return
        ligand_dict = {}
        if args.ligand_smiles:
            ligand_dict = json.loads(args.ligand_smiles)
        ...
        if not HAS_OPENMM:
            print("OpenMM required.")
            return
        cfg = RefinementConfig(steps=args.steps, openmm_platform=args.platform)
        engine = RefinementEngine(cfg)
        solute_coords = engine._setup_system(args.input)
        init_energy = openmm_solute_energy(solute_coords, engine.calculator).item()
        result = engine.refine(args.input, steps=args.steps)
        ...
        if final_energy < init_energy:
            print("✓ Energy decreased – refinement successful.")
        else:
            print("✗ Energy did not decrease – check system setup.")
