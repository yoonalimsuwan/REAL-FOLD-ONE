...
#   - Full‑atom physics powered by OpenMM:
...
#   - Scalable – chunked O(N) graphs, >100,000 atoms
...
def detect_optimal_device(verbose: bool = True) -> Tuple[torch.device, float]:
    ...
    if torch.cuda.is_available():
        try:
            ...
            if verbose:
                logger.info(f"✓ CUDA: {torch.cuda.get_device_name(device_id)} ({memory_gb:.1f} GB)")
...
class OpenMMSystemBuilder:
    ...
    def build_from_pdb(self, pdb_file: str, add_missing_residues: bool = True,
        ...
        # Step 1: Prepare topology (add missing residues, hydrogens, solvent, ions)
        ...
        # Detect non‑standard residues (now after solvation, but exclude solvent/ions)
        ...
        # Step 2: Build system (choose method based on non‑standard residues)
        if non_standard and HAS_OPENMMFORCEFIELDS:
            logger.info(f"Non‑standard residues detected: {non_standard}. Attempting SystemGenerator.")
            if not self.ligand_smiles:
                logger.warning("No ligand SMILES provided. Falling back to standard ForceField (may fail).")
                # Fallback to ForceField (will likely miss parameters for non‑standard)
                ...
                if self.implicit_solvent is not None:
                    ...
                    nb = PME if self.nonbonded_method == 'PME' else (CutoffPeriodic if self.nonbonded_method == 'CutoffPeriodic' else CutoffNonPeriodic)
                    system = ff.createSystem(modeller.topology,
                                             nonbondedMethod=nb,
                                             nonbondedCutoff=1.0 * unit.nanometer,
                                             constraints=HBonds if self.rigid_water else None)
            else:
                # Build Molecule objects from SMILES for the non‑standard residues
                molecules = []
                ...
                # implicit solvent is not directly applied via SystemGenerator; if requested, warn
                if self.implicit_solvent is not None:
            ...
            # Standard residues only (or openmmforcefields not available)
            ff = ForceField(*self.forcefield_files)
            if self.implicit_solvent is not None:
                system = ff.createSystem(modeller.topology,
                                         nonbondedMethod=NoCutoff,
        ...
        # Identify solute mask and Cα/C4' indices
...
# =============================================================================
# 3. Differentiable OpenMM Energy (solute‑only optimization)
# =============================================================================
class SoluteOpenMMEnergy(torch.autograd.Function):
    """Compute OpenMM energy over all atoms; gradient only for solute atoms."""
    @staticmethod
    def forward(ctx, solute_coords: torch.Tensor,
                full_coords_fixed: torch.Tensor, solute_mask: torch.Tensor,
                system: mm.System, topology: app.Topology, platform_name: str):
        full = full_coords_fixed.clone()
        full[solute_mask] = solute_coords
        pos_np = full.detach().cpu().numpy() * 0.1  # A → nm
        ...
        ctx.save_for_backward(forces_tensor[solute_mask])
        ctx.context = context
        return torch.tensor(energy_kcal, device=solute_coords.device, dtype=torch.float32)
...
def openmm_solute_energy(solute_coords: torch.Tensor, full_coords_fixed: torch.Tensor,
                         solute_mask: torch.Tensor, system: mm.System, topology: app.Topology,
                         platform: str = 'CUDA') -> torch.Tensor:
    return SoluteOpenMMEnergy.apply(solute_coords, full_coords_fixed, solute_mask,
                                    system, topology, platform)

# =============================================================================
# 4. Neighbor List Manager (for Cα‑based SOC terms)
# =============================================================================
...
class WireframeOrigami:
    ...
    def build_3d_model(self):
        ...
        for idx in range(len(path) - 1):
            ...
            vec = end - start
            dist = np.linalg.norm(vec)  # in Angstrom
            n_bp = max(1, int(dist / 3.38))  # bp rise ~3.38 Å
            ...
            helix_local = build_dna_helix("A" * n_bp)  # uses default rise=3.38 Å
...
def build_dna_helix(seq: str, rise: float = 3.38, twist: float = 36.0,
    ...
    """Build ideal B‑DNA helix coordinates. rise in Å, twist in degrees."""
    L = len(seq)
...
# =============================================================================
# 8. Antibody CDR Modelling & Interface Scoring (fully functional with OpenMM)
# =============================================================================
class RosettaScorer:
    """Binding score using OpenMM energy difference (complex – partners)."""
    def __init__(self, system: mm.System, topology: app.Topology,
        ...
        """
        Parameters:
          system, topology, full_coords_fixed, solute_mask – from the complex system.
          ab_solute_indices – indices (within solute) for antibody atoms.
          ag_solute_indices – indices (within solute) for antigen atoms.
        """
        self.system = system
    ...
    def _compute_energy(self, solute_coords: torch.Tensor) -> float:
        with torch.no_grad():
            return openmm_solute_energy(solute_coords, self.full_coords_fixed,
                                        self.solute_mask, self.system, self.topology,
                                        self.platform).item()

    def score_complex(self, coords_ab: torch.Tensor, coords_ag: torch.Tensor) -> float:
        """
        Compute binding energy = E(complex) – E(antibody alone) – E(antigen alone).
        All energies evaluated with solvent fixed.
        """
        # Combine into full solute array
        ...
        # Antibody alone: set antigen to its initial (unbound) coordinates
        ...
        # Antigen alone: set antibody to its initial coordinates
...
class CDRLoopModeler:
    """Monte Carlo remodelling of a CDR loop using OpenMM energy."""
...
class RefinementConfig:
    # Device
    device: str = 'auto'          # 'auto', 'cuda', 'cpu', 'mps', etc.
    # SOC
    w_soc: float = 0.3
    ...
    box_padding: float = 1.0
    implicit_solvent: Optional[str] = None
    # Multimer & drug design
    refine_solute_only: bool = True
    # Ligand SMILES for non‑standard residues
...
class RefinementEngine(nn.Module):
    def __init__(self, cfg: Optional[RefinementConfig] = None):
        ...
        # OpenMM components will be set during refine
        self.system = None
        self.topology = None
    ...
    def refine(self, pdb_file: str, steps: Optional[int] = None,
        ...
        sigma_val = torch.tensor(1.0)  # placeholder
        ...
        for step in range(steps):
            ...
            optimizer.zero_grad()
            # CA coords (leaf tensor for avalanche)
            ca = self._get_ca_coords(solute_coords)
            ca.retain_grad()

            with autocast(enabled=self.cfg.use_amp and self.device.type == 'cuda'):
                # OpenMM energy (solute gradient only)
                E_openmm = openmm_solute_energy(
                    solute_coords, self.full_coords_fixed, self.solute_mask,
                    self.system, self.topology, self.cfg.openmm_platform
                )
                # SOC energy on CA
            ...
            # SOC avalanche gradient (uses ca.grad, which now exists)
            ...
            # Adaptive Langevin noise (only if enabled)
            sigma_val = self.soc.sigma(ca.detach())
            T_val = self.soc.temperature(sigma_val)
            if self.cfg.use_langevin:
                noise_scale = math.sqrt(2 * self.cfg.friction * T_val.item() / 300.0) * self.cfg.lr
            ...
            if self.rg is not None and step > 0 and step % self.cfg.rg_interval == 0:
                ...
                solute_coords.data[self.ca_indices] += displacement
    ...
    def create_cdr_scorer(self, ab_chain_id: str, ag_chain_id: str) -> RosettaScorer:
        """
        Create a RosettaScorer for antibody‑antigen interface scoring.
        Requires that the loaded system contains the specified chains.
        """
        ...
        return RosettaScorer(
            ...
            platform=self.cfg.openmm_platform
        )
...
class Trainer:
    ...
    def train(self, pdb_list: List[str], epochs: int = 100, lr: float = 1e-3):
        ...
        for fpath in pdb_list:
            try:
                solute_coords = self.engine._setup_system(fpath)
                structures.append(solute_coords)
            except Exception as e:
                logger.warning(f"Skipping {fpath}: {e}")
        optimizer = torch.optim.Adam(self.kernel.parameters(), lr=lr)
        for epoch in range(epochs):
            total_loss = 0.0
            for solute_coords in structures:
                solute_coords = solute_coords.clone().detach().requires_grad_(True)
                ca = self.engine._get_ca_coords(solute_coords)
                M = ca.shape[0]
                alpha = torch.ones(M, device=solute_coords.device)
                edge = self.engine.neighbor_mgr.build(ca)['soc']
                E_soc = self.engine.soc.compute_soc_energy(ca, alpha, edge[0], edge[1],
                                                           w_soc=self.cfg.w_soc)
                E_openmm = openmm_solute_energy(solute_coords, self.engine.full_coords_fixed,
                                                self.engine.solute_mask, self.engine.system,
                                                self.engine.topology, self.cfg.openmm_platform)
                loss = E_openmm + E_soc
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
...
def main():
    ...
    refine_parser.add_argument('--platform', default='CUDA', help='OpenMM platform')
    ...
    if args.command == 'refine':
        ...
        E = openmm_solute_energy(solute_coords, engine.full_coords_fixed, engine.solute_mask,
                                 engine.system, engine.topology)
        E.backward()
