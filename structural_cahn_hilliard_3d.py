...
class PhaseFieldCrystal3D(StructuralCahnHilliard3D):
    ...
    def _step_pfc_imex(
        ...
        The PDE is ``du/dt = Delta_S(mu_PFC)`` with
            mu_PFC = (r*u + u^3) + u + 2*Delta_S(u) + Delta_S^2(u)
        i.e. mu_PFC = f'(u) + (1 + Delta_S)^2 u, where f(u) = r/2*u^2 + 1/4*u^4.

        Only the cubic/quartic term f'(u) = r*u + u^3 is genuinely
        nonlinear; everything else is linear in u and, for the
        homogeneous (sigma ~ sigma_mean) part, diagonal in Fourier space.
        Treating the FULL linear part implicitly via backward Euler on
            du/dt = Delta_S[f'(u)] + Delta_S(1 + Delta_S)^2 u
        gives, in Fourier space (Delta -> -sigma_mean*k^2 for the
        homogeneous part):
            du/dt|_linear -> -sigma_mean*k^2 * (1 - sigma_mean*k^2)^2 * u_hat
                            = -sigma_mean*(k^2 - 2*sigma_mean*k^4 + sigma_mean^2*k^6) * u_hat
        so the implicit denominator is
            denom = 1 + dt*M*sigma_mean*(k^2 - 2*sigma_mean*k^4 + sigma_mean^2*k^6)
        (previously this had +k^4 instead of -2*sigma_mean*k^4, which
        silently produced incorrect PFC dynamics whenever scheme="imex"
        was selected; scheme="explicit", the default, was unaffected).

        The remaining inhomogeneous correction (sigma varying in space)
        and the genuine nonlinearity f'(u) are treated explicitly.
        """
        cfg        = self.cfg
        nx, ny, nz = u.shape
        dev, dtype = u.device, u.dtype
        ...
        # Linear operator (1 + Delta_S)^2 u = u + 2*Delta_S(u) + Delta_S^2(u),
        # applied via the actual (possibly spatially-varying) structural
        # Laplacian, then Delta_S of the WHOLE mu_PFC gives the true RHS.
        lap1 = self._structural_laplacian(u, sigma)                  # Delta_S(u)
        lap2 = self._structural_laplacian(lap1, sigma)                # Delta_S^2(u)
        mu_pfc_linear_part = u + 2.0 * lap1 + lap2                    # (1+Delta_S)^2 u
        mu_pfc_nonlin_part = r * u + u**3                              # f'(u)

        # Full real-space RHS: Delta_S applied to the complete mu_PFC
        # (both the nonlinear f'(u) term and the linear (1+Delta_S)^2 u
        # term). This is divided by the implicit spectral denominator
        # below, which represents (1 - dt*M*L_eig) for the homogeneous
        # linear part L_eig — a standard linearly-implicit / IMEX-via-
        # division scheme: as dt -> 0, denom -> 1 and this reduces
        # exactly to forward Euler on du/dt = Delta_S(mu_PFC); for finite
        # dt, the stiff (1+Delta_S)^2 contribution is implicitly damped
        # via the denominator rather than amplified, giving unconditional
        # linear stability for the homogeneous (sigma ~ sigma_mean) case.
        explicit_rhs = self._structural_laplacian(mu_pfc_nonlin_part + mu_pfc_linear_part, sigma)

        u_hat       = torch.fft.rfftn(u,            norm="ortho")
        explicit_hat = torch.fft.rfftn(explicit_rhs, norm="ortho")

        k4    = k2 ** 2
        k6    = k2 ** 3
        denom = 1.0 + cfg.dt * cfg.mobility * sigma_mean * (
            k2 - 2.0 * sigma_mean * k4 + (sigma_mean ** 2) * k6
        )
        u_new_hat = (u_hat + cfg.dt * cfg.mobility * explicit_hat) / denom
