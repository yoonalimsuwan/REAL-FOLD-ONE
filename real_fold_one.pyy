...
class OpenMMSystemBuilder:
    ...
    def build_from_pdb(self, pdb_file: str, add_missing_residues: bool = True,
        ...
        if non_standard and HAS_OPENMMFORCEFIELDS and ligand_smiles:
            ...
            # Choose nonbonded method and cutoff
            if self.implicit_solvent is not None:
                # Implicit solvent uses GB, no PME
                nonbonded_method = NoCutoff if self.nonbonded_method == 'NoCutoff' else CutoffNonPeriodic
            else:
            ...
            system = ff.createSystem(modeller.topology,
                                     ...
                                     implicitSolvent=None)  # will be set later if needed

        # Implicit solvent setup
        if self.implicit_solvent is not None:
            # Add GB force
            gb_force = mm.GBSAOBCForce() if self.implicit_solvent.lower() == 'obc' else \
                       mm.GBn2Force() if self.implicit_solvent.lower() == 'gbn2' else \
                       mm.CustomGBForce()
            # Set salt concentration
            if hasattr(gb_force, 'setSoluteDielectric'):
                gb_force.setSoluteDielectric(1.0)
            if hasattr(gb_force, 'setSolventDielectric'):
                gb_force.setSolventDielectric(78.5)
            if hasattr(gb_force, 'setSurfaceAreaEnergy'):
                gb_force.setSurfaceAreaEnergy(0.0042)  # kJ/mol/A^2
            if hasattr(gb_force, 'setCutoff'):
                gb_force.setCutoff(1.0)
            system.addForce(gb_force)
            # Remove explicit water if any (should not be added)
            if solvate:
                logger.warning("Implicit solvent selected; skipping explicit solvation.")
            solvate = False
            add_ions = False  # Implicit solvent already accounts for ionic strength via GB model
...
def scipy_radius_graph(coords: torch.Tensor, r: float,
    ...
    # Keep only pairs where i < j (upper triangle) for consistency (tree returns both orders? Actually query_pairs returns each pair once, sorted)
    return edge, dist
...
class RosettaScorer:
    ...
    def score_complex(self, coords_ab: torch.Tensor, coords_ag: torch.Tensor) -> float:
        ...
        # Build a full solute tensor from the two parts (assume order matches chain selection)
        # This simplistic approach works if the order of atoms in the complex PDB matches
        # the concatenation of antibody then antigen.  In practice we should align atom indices;
        # here we trust that the same PDB order is used.
...
class RefinementEngine(nn.Module):
    ...
    def create_cdr_scorer(self, ab_chain_id: str, ag_chain_id: str,
        ...
        pdb = pdb_file or (self.pdb_file if hasattr(self, 'pdb_file') else None)
        if pdb is None:
