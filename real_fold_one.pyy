...
# =============================================================================
class PME:
    def __init__(self, grid_spacing=0.75, interpolation_order=4, alpha=None):
...
class MultigridPoisson:
    def __init__(self, finest_spacing=0.5, levels=3):
    ...
    def _smooth(self, u, f, nu=2, omega=2/3):
        for _ in range(nu):
            u_shift = (torch.roll(u, -1, dims=0) + torch.roll(u, 1, dims=0) +
                       torch.roll(u, -1, dims=1) + torch.roll(u, 1, dims=1) +
                       torch.roll(u, -1, dims=2) + torch.roll(u, 1, dims=2))
            u = (1-omega)*u + (omega/6)*(u_shift + f)
        return u
    ...
    def solve(self, coords, q):
        ...
        n_f = self.grids[0]
        h = self.box[0,0] / n_f
        # Discretized Poisson: -Δ_h u = f, with f = 4π ρ h².
        rho_f = self._charge_grid(coords, q, n_f, self.inv_box) * (4 * math.pi / h)
...
class BlockwiseLongRange:
    ...
    def compute_energy(self, coords, q, direct_edge, direct_dist):
        ...
        # dipole computed relative to block center
        rel_coords = coords - block_centers[block_numbers]
        for d in range(3):
            block_dipole[:, d].scatter_add_(0, block_numbers, q * rel_coords[:, d])
...
def _map_ref(idx, cur_len):
    if idx <= 0:
        return max(0, cur_len + idx)
    else:
        return min(idx, cur_len-1)
...
def build_sidechain_atoms(ca, seq, chi_angles):
    ...
    for i, aa in enumerate(seq):
        ...
        for (name, typ, parent, bond_len, ang_deg, ref, dih0) in topo:
            cur_len = len(local_atoms)
            a = _map_ref(ref[0], cur_len)
            b = _map_ref(ref[1], cur_len)
            c = _map_ref(ref[2], cur_len)
...
def build_single_nucleotide_ic(C4_prime, prev_C4, next_C4, nt_type):
    ...
    for name, typ, parent, blen, ang, ref, dih0 in topo:
        if parent == 0 and len(coords)==0:
            ...
            a,b,c = ref
            cur_len = len(coords)
            aa = _map_ref(a, cur_len); bb = _map_ref(b, cur_len); cc = _map_ref(c, cur_len)
            pa = coords[aa] if aa<cur_len else ppos
            pb = coords[bb] if bb<cur_len else ppos
            pc = coords[cc] if cc<cur_len else ppos
...
# =============================================================================
# 9. Energy Terms (Protein) – extensive (sum) versions
# =============================================================================
...
def alpha_regularisation(alpha, w_ent=0.5, w_smooth=0.1):
    ent = -(alpha * torch.log(alpha+1e-8)).sum()
...
def validate_gradients(engine, coords, sequence, eps=1e-4):
    ...
    edge_dict = engine._build_edges(coords)
    ...
    alpha = torch.ones(len(sequence), device=coords.device)
    chi = torch.zeros(len(sequence), MAX_CHI, device=coords.device)
    chain_types = ['protein']*len(sequence)
    prot_mask = torch.ones(len(sequence), dtype=torch.bool, device=coords.device)
    dna_mask = torch.zeros(len(sequence), dtype=torch.bool, device=coords.device)
    mask = prot_mask
    E = engine._total_energy(coords, sequence, alpha, chi,
                             edge_dict, edge_hb, edge_hb_dist,
                             [], chain_types, mask, None)
    E.backward()
    ...
    for i in range(coords.shape[0]):
        for j in range(3):
            ...
            coords[i,j] = orig + eps
            edge_dict_plus = engine._build_edges(coords)
            atoms_plus = reconstruct_backbone(coords)
            edge_hb_plus, edge_hb_dist_plus = NeighborListManager({'hb':3.5}, engine.cfg.max_neighbors).build(atoms_plus['O'])['hb']
            E_plus = engine._total_energy(coords, sequence, alpha, chi,
                                          edge_dict_plus, edge_hb_plus, edge_hb_dist_plus,
                                          [], chain_types, mask, None)
            coords[i,j] = orig - eps
            edge_dict_minus = engine._build_edges(coords)
            atoms_minus = reconstruct_backbone(coords)
            edge_hb_minus, edge_hb_dist_minus = NeighborListManager({'hb':3.5}, engine.cfg.max_neighbors).build(atoms_minus['O'])['hb']
            E_minus = engine._total_energy(coords, sequence, alpha, chi,
                                           edge_dict_minus, edge_hb_minus, edge_hb_dist_minus,
                                           [], chain_types, mask, None)
...
# =============================================================================
# 15. Refinement Engine (with Temperature Scheduling & Training)
# =============================================================================
@dataclass
...
class RefinementEngine:
    ...
    def _temperature_schedule(self, step, total_steps):
        ...
        # Derive per‑chain‑type masks
        prot_mask = torch.tensor([t == 'protein' for t in chain_types], device=ca.device, dtype=torch.bool)
        dna_mask  = torch.tensor([t in ('dna','rna') for t in chain_types], device=ca.device, dtype=torch.bool)

        # Global residue mask (user‑supplied) is applied on top of all chain masks
        if mask is not None:
            prot_mask = prot_mask & mask
            dna_mask  = dna_mask & mask

        # Protein‑specific terms (only where prot_mask is True)
        if prot_mask.any():
            E += energy_bond(ca, alpha, self.cfg.w_bond, mod=0.1,
                             target=self.cfg.bond_target, mask=prot_mask)
            E += energy_angle(ca, alpha, self.cfg.w_angle, mod=0.05,
                              target_rad=self.cfg.angle_target_rad, mask=prot_mask)
            E += energy_rama(phi, psi, seq, alpha, self.cfg.w_rama, mod=0.2,
                             mask=prot_mask)
            E += energy_clash(ca, alpha, edge_dict['clash'][0], edge_dict['clash'][1],
                              self.cfg.w_clash, self.cfg.clash_radius, mod=0.1,
                              mask=prot_mask)
            E += energy_hbond(atoms['O'], atoms['N'], atoms['C'], alpha,
                              edge_hb, edge_hb_dist, self.cfg.w_hbond, mod=0.1,
                              mask=prot_mask)

            # Protein electrostatics (residue‑level) – only on protein atoms
            elec_edge, elec_dist = edge_dict['elec']
            # We need a protein‑only edge list; easiest is to apply mask via energy function
            q_res = torch.tensor([RESIDUE_CHARGE.get(a,0.0) for a in seq], device=ca.device)
            if self.cfg.use_pme:
                E += self.pme.energy(ca, q_res * prot_mask.float())  # charge only on protein
            elif self.cfg.use_multigrid:
                E += self.mg.solve(ca, q_res * prot_mask.float())
            else:
                E += energy_electro(ca, seq, elec_edge, elec_dist, self.cfg.w_electro,
                                    mask=prot_mask)
                if self.block_lr:
                    E += self.block_lr.compute_energy(ca, q_res * prot_mask.float(),
                                                      elec_edge, elec_dist)

            E += energy_solvent(ca, seq, elec_edge, self.cfg.w_solvent, mask=prot_mask)
            E += self.soc.compute_soc_energy(ca, alpha, elec_edge, elec_dist,
                                             mask=prot_mask, w_soc=self.cfg.w_soc)
            E += alpha_regularisation(alpha, self.cfg.w_alpha_entropy, self.cfg.w_alpha_smooth)

        # DNA/RNA‑specific terms (only where dna_mask is True)
        if dna_mask.any():
        ...
        # Full‑atom protein sidechain + LJ/Coulomb (only on protein parts)
        if prot_mask.any() and chi is not None:
        ...
        # Ligand interactions (only with protein atoms)
        ...
        # Global chain break energy (applies across all chains)
    ...
    def refine(self, coords, sequence, mask=None, chain_types=None, chain_boundaries=None,
        ...
        for step in range(steps):
            ...
            if self.cfg.use_milstein:
                pass  # simplified additive noise
            with torch.no_grad():
...
# =============================================================================
# 16. Training Module – Learnable SOC kernel and energy weights
# =============================================================================
class Trainer:
        ...
        for epoch in range(epochs):
            ...
            for coords_np, seq in structures:
                ...
                if use_full_atom:
                    ...
                    chain_types = ['protein']*len(seq)
                    mask = torch.ones(len(seq), dtype=torch.bool, device=self.engine.device)
