...
class NetlistExporter:
    ...
    def export_spice(
        ...
        """
        Write a real SPICE deck (.cir) for classical platforms
        (CMOS_DIGITAL, RF_ANALOG, MEMS). Aggregates the per-cell parameter
        grids into a single representative lumped RLC section per node,
...
class StructuralToQEDABridge:
    ...
    def process_simulation_result(
        ...
        if platform in (ChipPlatform.CMOS_DIGITAL, ChipPlatform.RF_ANALOG, ChipPlatform.MEMS):
