...
try:
    from real_fold_one import (
        ...
        HAS_BIOTITE,
        OPTIMAL_DEVICE,
        # Internal classes we need for the coarse energy model
        CSOCKernel,
        NeighborListManager,
        HYDROPHOBICITY,
        WC_PAIRS,
...
class HTRefinementEngine(RefinementEngine):
    ...
    def _pairwise_type_energy(self, ca: torch.Tensor, sequence: str,
        ...
        seq_types = [detect_sequence_type(s) for s in chain_types]
        # Flat type map per residue
        type_seq = []
        for s, ct in zip(sequence, seq_types):
            type_seq.append(ct)
        # Energy accumulator
        E_type = 0.0
